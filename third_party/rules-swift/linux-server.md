...
Load on demand. Triggers: Linux, server, Vapor, Hummingbird, AsyncHTTPClient, swift-log, swift-crypto, FluentSQLite, FluentPostgres, NIO, SIGTERM, graceful shutdown, Docker, `swift:6.2-jammy`, `swift:6.2-noble`, Bundle.module, FoundationNetworking on a server, isatty, BoringSSL.
...
**Docker image choice:**
- `swift:6.2-jammy`, Ubuntu 22.04 LTS, the default for most production deploys.
- `swift:6.2-noble`, Ubuntu 24.04 LTS, newer.
- `swift:6.2-jammy-slim`, runtime-only base for the production stage.
- `swift:6.2-bookworm`, Debian 12, for some legacy deploys.
- `swift:6.2-alpine`, Alpine + Musl, smallest image. Test Musl conditionals if you use this.
...
A Linux-buildable repo conforms when each rule above is shown to hold:

- **No Apple-only frameworks in server code.** The server/CLI targets import none of CoreData, an unguarded CryptoKit, or os.log on the server tier:

```bash
grep -rnE 'import CoreData|import CryptoKit|import os(\.log)?' Sources/<server-target>/
```

  Output MUST be empty for the server target, save a `CryptoKit` import that sits inside a `#if canImport(CryptoKit)` guard (§4).

- **Every URLSession is invalidated before release.** If any survives, each is paired with `invalidateAndCancel()` in a `deinit` or a `defer` (§1):

```bash
grep -rn 'URLSession(' Sources/
```

  Every hit MUST have a matching `invalidateAndCancel()` in scope; an unpaired session is the Linux deinit crash this rule prevents.

- **A Linux build runs on every PR.** A CI job builds the Linux product in a `swift:6.2-*` container (§11):

```bash
grep -rnE 'swift:6\.0-(jammy|noble|bookworm|alpine)' .github/workflows/
```

  Output MUST be non-empty, and the job MUST run `swift build` for the named Linux product.

- **Observable checklist** for the patterns no single grep settles, each phrased so two reviewers agree: POSIX interop uses the `canImport(Darwin)/Glibc/Musl` conditional (or `swift-system`), never a bare `import Glibc`; startup resources are compiled to Swift rather than read through `Bundle.module` (§6); `swift-log`'s `Logger` and `os.log`'s `Logger` never appear in the same source file (§2); cross-platform `FileHandle` writes go through the §8 wrapper or `swift-system`; and the manifest splits Apple-only products/targets behind `#if os(iOS) || os(macOS)` while server products stay unconditional (§12).
