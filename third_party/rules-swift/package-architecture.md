...
How to decompose Swift code into focused SwiftPM targets within the repository's
single active package manifest. In a package repository that manifest is
`Package.swift` at the repository root. In a project monorepo that manifest is
`Packages/Package.swift`.

The pattern: single-responsibility SwiftPM targets with explicit,
unidirectional dependencies, all in one active manifest. Each target has one
cohesive job and declares exactly what it depends on. This buys isolated
compilation, parallel builds, a clear dependency graph, and targets you can test
and lift out in isolation.

## What this covers

Add a new target to the active package manifest when one of these is true:
...
### Rule 1: Single responsibility per target

Create targets with one clear purpose:

- One well-defined responsibility per target.
...
### Rule 3: Target granularity

Prefer smaller, focused targets over larger ones:

- Single-file targets are fine when the unit is genuinely standalone (a color foundation, a single protocol, a single transport).
- Separate by role, not by topical bundling: foundation primitives, infrastructure, protocols, middleware, services, per-feature or per-verb operation targets, front-door binaries.
- Each cohesive responsibility (a feature, a CLI verb, a middleware, a transport, a service) gets its own target.
...
- **Per-feature or per-verb operation targets:** one target per user-facing flow (in a UI app) or one per CLI verb (in a CLI). Name after the responsibility, not after a role suffix.
- **Service targets:** `*Service`, or a `Services` aggregator for cross-layer read/write services consumed by multiple front-doors.
- **Aggregators:** umbrella targets only when the umbrella adds real value (a preview host, an all-features aggregator for app composition). Do not add one by default.

Keep the established public library name as the public anchor and grow new
targets around it rather than renaming it.

### Rule 5: Layer architecture (unidirectional)

Organize targets into layers with dependency flow strictly bottom to top. The exact number of layers varies per project; the constants are:
...
Every dependency edge points upward. No back-edges. The active layer instantiation lives in the repository's active `Package.swift`.
...
│   └─ YES → add to the shared foundation target (e.g. SharedModels)
│
...
│   └─ YES → create a new feature / verb target
│
├─ Is it infrastructure (transport, parser, persistence, middleware)?
│   └─ YES → create or extend the matching infrastructure target
│
└─ Still unsure?
    └─ Ask: "Could this be reused or tested in isolation?"
        ├─ YES → create a new target
        └─ NO  → add to the most specific existing target
```

### Create a new target when
...
### Do NOT create a new target when

1. **It is single use**, only used by one feature and tightly coupled to it. Add it to that feature's target.
2. **It is a trivial helper** of one or two functions with no external dependencies. Add it to an existing utility target.
3. **It is temporary** (a spike or proof of concept). Keep it in the feature until it proves stable.

## Decision checklist

Before creating a new target:
...
- [ ] Product registered in the `Package.swift` products array when the target is public or executable
- [ ] Dependencies only flow upward

Before adding to an existing target:

- [ ] The new code shares a responsibility with what is there
- [ ] No better-suited target exists
- [ ] You are not creating a "god target" with mixed concerns
- [ ] You are not introducing unwanted dependencies onto the target's consumers
...
```
Sources/                         # or Packages/Sources/ in a project monorepo
