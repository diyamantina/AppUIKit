...
How a TileKit-style library organizes its documentation: a single DocC catalog that is the source of truth and moves with the code in every PR.
...
- MUST use plain markdown paths (`Sources/Foo/README.md` or `Packages/Sources/Foo/README.md`, matching repository shape) for files outside the catalog.
...
- No absolute machine paths such as user home directories or mounted-volume paths.
...
- [ ] No private or user-specific references leaked (user home directories, mounted-volume paths, etc.).
