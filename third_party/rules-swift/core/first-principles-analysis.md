...
A first-principles analysis artifact conforms when it is auditable from the text itself:

1. It has a prerequisites or glossary section for the language, libraries, protocols, formats, data model, and domain terms the rest of the document uses.
2. Each non-trivial algorithm, parser, transformation, or workflow has at least one concrete worked input and output.
3. It includes an end-to-end worked example that follows one real input through every relevant component to the final output.
4. It closes with a reimplementation roadmap or equivalent porting plan that names phase order and the hard parts.
5. Every numeric or factual claim is tagged as `MEASURED`, `DERIVED`, `RANGED`, `STRUCTURAL`, or `DOCUMENTED`, with the cited command, formula, file path, or source beside the claim.
6. None of the forbidden softeners appear next to an empirical claim unless the claim is tagged with reproducible evidence.

Runnable spot checks:

```sh
rg -n 'typically|usually|approximately|roughly|~[0-9]|tens of|a few hundred|around [0-9]|on the order of' docs/
rg -n '[0-9]' docs/
```

The first command should return no untagged empirical claims. The second command is a review queue: every surviving numeric or factual claim in the touched artifact must carry one of the five claim tags or be an identifier, code literal, date, version, or other non-empirical token.
