...
Start CI tests on GitHub-hosted runners when the suite can run there. Escalate to a self-hosted runner only after the hosted failure is diagnosed as a runner limitation: missing SDK/toolchain, required hardware or architecture, private network or secret access, Docker/toolchain setup that hosted runners cannot support, runner minutes, cache size, or timeout. A real product failure on a hosted runner must be fixed, not hidden by moving the job.
