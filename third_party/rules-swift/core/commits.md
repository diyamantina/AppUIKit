...
- Self-referential phrasing ("as an AI", "the agent did X", "automated cleanup") and any coded wink at tool authorship.
- Stylistic tells: emoji used as a signature or flourish (notably the 🤖 robot and ✨ sparkle), a trailing "generated" / "powered by", or watermarks (zero-width characters, hidden comments naming a tool, `-ai` / `-bot` / tool-name branch suffixes).
