...
| `oracle-first.md` | Build the answer key before the system: define the Oracle, comparison law, failure report, unsupported cases, and parity gate before production work. |
...
Point your agent (or yourself) at this directory as always-loaded context. The ethic rules (`no-shortcuts-first-principles`, `oracle-first`, `proof-discipline`, `verification`, `testing-discipline`) apply on every task that makes or evaluates a correctness claim. The process rules (`brainstorming`, `writing-plans`, `commits`, `file-naming`) apply when their topic comes up.
