...
`oracle-first.md` (build the answer key before the system),
`no-shortcuts-first-principles.md` (disclosed limits, never hidden debt), and
...
- `oracle-first.md`: the upstream design discipline. It defines the answer key,
  comparison law, and failure report this rule later labels as theorem, witnessed,
  sampled, assumed, or blocked.
