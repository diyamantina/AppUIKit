...
### CI runner escalation

GitHub-hosted Actions are the default runner tier for portable gates. A self-hosted runner is still GitHub Actions, but it is a separate escalation tier that runs the workflow on owned hardware. Do not blur the two in workflow names, badges, or failure reports.

- MUST start with GitHub-hosted runners for gates that can run on `ubuntu-latest`, `macos-latest`, or `windows-latest`.
- MUST NOT move a failing gate to a self-hosted runner until the failure is diagnosed as a hosted-runner limitation, not a product bug, test failure, missing dependency declaration, or flaky harness.
- MAY use self-hosted runners when the gate needs hardware, SDKs, architecture, secrets, local network access, Docker/toolchain setup, cache size, runner minutes, or wall-clock limits that hosted runners cannot provide.
- MUST name or comment the self-hosted reason in the workflow, especially for jobs using labels such as `runs-on: [self-hosted, macos, claw]`.
- MUST keep the same verification contract on self-hosted runners: run the real format, lint, build, test, oracle, or parity gate and report the same pass/fail evidence a hosted runner would.

WRONG: `swift test` fails on `macos-latest`, so change the job to `runs-on: [self-hosted, macos, claw]` and call the build fixed.

RIGHT: reproduce the hosted failure, identify the hosted-runner blocker (for example missing Swift SDK, Xcode version, Apple Silicon requirement, private dependency access, Docker/Swift SDK setup, or timeout), then add or move to a self-hosted job with a workflow comment naming that blocker.

Acceptance check: every self-hosted workflow has an explicit `self-hosted` runner label and a nearby comment, job name, or README/badge text that says why hosted runners are insufficient. A self-hosted job that only makes a red hosted test disappear without a diagnosis fails this rule.
