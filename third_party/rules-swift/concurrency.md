...
Conforms when each binding clause above is shown to hold:

- **Every escape hatch is justified.** Every `@unchecked Sendable` and every `nonisolated(unsafe)` carries a one-line comment beside or above it explaining why the race is acceptable. List them and read the neighbouring line:

```bash
grep -rnE '@unchecked Sendable|nonisolated\(unsafe\)' Sources/ Tests/
```

  Every hit MUST have an adjacent justifying comment; an unjustified one is a violation.

- **The anti-patterns are absent from new code.** The following finds no lines added by the change under review:

```bash
grep -rnE 'DispatchQueue\.main\.async|NSLock|pthread_mutex|XCTestExpectation' Sources/ Tests/
```

  Each surviving hit must be a deliberate legacy-bridging exception named in a comment (the `withCheckedContinuation` / `XCTestExpectation` "only when wrapping legacy callback APIs" carve-out), not a default reach.

- **Observable checklist** for the clauses no grep settles, each phrased so two reviewers agree: public value types on the API surface declare `: Sendable` explicitly and error enums always do; `actor` appears only for cross-task shared mutable state, not for data that could be a copied struct; `@MainActor` does not appear in a non-UI CLI/library target to "make it compile"; `withTaskGroup` calls are bounded, never unbounded; and no new type pairs a hand-rolled lock with `@unchecked Sendable` (the "use `actor`" clause).
...
- Point-Free Dependencies (within-module, closure-based with `@Sendable` closures) is an optional dependency-wiring choice, not required by these rules; plain constructor injection is the alternative.
