...
Conforms when the manifest's three top-level arrays are built the closure-with-local-variables way, not as inline literals:

- **Each array opens a closure, not a bracket.** `deps`, `allProducts`, and `targets` read `let <name>: [...] = {` and return their grouped concatenation:

```bash
grep -nE 'let (deps|allProducts|targets)[^=]*=[[:space:]]*\[' Packages/Package.swift
```

  Output MUST be empty. A hit means an array was written as an inline `[...]` literal, the anti-pattern this file replaces.

- **No targets defined inline in the returned array.** Each target is a named intermediate variable (`let sharedModelsTarget = Target.target(...)`), grouped by layer, then concatenated in the `return`. Confirm by reading the `targets` closure: the `return` line is a sum of group arrays (`return modelTargets + apiTargets + ...`), not a literal full of `Target.target(...)` calls.

- **Observable checklist**, each a clear pass/fail: comment headers separate the groups (`// ---------- Shared Models ----------`); every variable has a descriptive name rather than an inline expression; and arrays use trailing commas for clean diffs. A manifest whose `deps`, `allProducts`, or `targets` is a bare inline array fails this rule even if it compiles.
