...
The `withDependencies` overrides shown in the patterns below assume the Point-Free Dependencies library. Where a project does not use that library, control collaborators through plain constructor injection instead; the test-isolation and determinism rules apply either way.
...
Source and test targets mirror each other under the active package root.
Package repositories use root `Sources/` and `Tests/`; project monorepos use
`Packages/Sources/` and `Packages/Tests/`.
...
Public test doubles (mocks, fakes, stubs) are placed in
`Tests/<SourceTarget>Tests/Mocks/` or
`Packages/Tests/<SourceTarget>Tests/Mocks/`, matching the repository shape.
They are never placed in `Sources/<SourceTarget>/` or
`Packages/Sources/<SourceTarget>/`. Mocks shipped from source targets leak into
production binaries.
