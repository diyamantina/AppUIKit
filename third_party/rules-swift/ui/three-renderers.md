...
**Status: the standing UI aspiration, pursued whenever bandwidth allows, and subordinate to
[`native-per-platform.md`](native-per-platform.md).** Build the
...
This rule applies only where the same UI is also the best native UI on each platform.
Where building the same surface in all three SDKs would trade away the native experience for
write-once convenience, `native-per-platform.md` governs: optimize for the user, and let the
per-platform UIs diverge. "All three renderers" is the goal for the shared-and-still-native case
and a comparison instrument, never a mandate to flatten the platforms into one layout.
