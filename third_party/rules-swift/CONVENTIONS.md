...
The Swift coding conventions for Tiledown. They keep the codebase consistent,
...
These are the **Swift domain** conventions: the language- and platform-specific
craft. They sit on top of a vendored **core spine** of cross-cutting engineering
discipline (the optimal-path ethic, proof and verification, testing discipline,
systematic debugging, commits, and git process). That spine lives in
[`core/`](core/); read it first, the Swift rules build on it and do not
repeat it.

This page is the overview. The full per-area Swift rules live at the repository
root (index at [`README.md`](README.md)), and the cross-cutting rules in
[`core/`](core/).
...
Tiledown builds and runs on Apple platforms and Linux, with Windows where explicitly noted. The TileKit engine itself targets macOS and Linux; the broader cross-platform discipline (and the CI matrix in [cross-platform.md](cross-platform.md)) extends to Windows for code that opts into it.
...
The discipline lives in [`core/verification.md`](core/verification.md): no
claim of done, fixed, or passing without fresh command output in the same response.
For Swift, match the command to the claim:
...
See [`core/commits.md`](core/commits.md) and
[`core/git-discipline.md`](core/git-discipline.md), and the project's
[CONTRIBUTING.md](CONTRIBUTING.md). In short: Conventional Commits, one focused
change per PR, a `CHANGELOG.md` entry for any change touching shipping source, and
no AI attribution or em dashes in any committed text.
