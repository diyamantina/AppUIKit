...
v=$(grep -rniE "$VEND" "$ROOT" --include='*.md' 2>/dev/null | grep -vE '/\.git/|/\.build/|/third_party/' \
      | grep -vE '(^|[./])(commits|git-discipline|rules-swift-audit)\.md:' || true)
...
p=$(grep -rnE '/Users/|/Volumes/' "$ROOT" --include='*.md' 2>/dev/null | grep -vE '/\.git/|/\.build/|/third_party/' || true)
[ -n "$p" ] && { echo "LEAK GATE FAIL (machine-absolute path):"; echo "$p" | head; fail=1; }

# No em dashes in any tracked markdown.
e=$(grep -rlF "$(printf '\xe2\x80\x94')" "$ROOT" --include='*.md' 2>/dev/null | grep -vE '/\.git/|/\.build/|/third_party/' || true)
[ -n "$e" ] && { echo "LEAK GATE FAIL (em dash):"; echo "$e"; fail=1; }

[ "$fail" = 0 ] || { echo "LEAK GATE FAILED for $ROOT" >&2; exit 1; }
echo "leak gate clean ($(find "$ROOT" -name '*.md' -not -path '*/.git/*' -not -path '*/.build/*' -not -path '*/third_party/*' | wc -l | tr -d ' ') md files)"
